            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        # Merge auxiliary metrics from reward function if available
                        try:
                            aux = getattr(self.reward_fn, "latest_metrics", None)
                            if isinstance(aux, dict) and aux:
                                metrics.update(aux)
                                try:
                                    bd = {k: float(v) for k, v in aux.items() if str(k).startswith("reward_breakdown/")}
                                    if bd:
                                        batch.meta_info["reward_breakdown"] = bd
                                except Exception:
                                    pass
                        except Exception:
                            pass
                        # Attach optional reward breakdown tensors (e.g., x1/x2/x3) for debugging / grad attribution
                        try:
                            reward_tensors = getattr(self.reward_fn, "latest_reward_tensors", None)
                            if isinstance(reward_tensors, dict) and reward_tensors:
                                for name, tensor in reward_tensors.items():
                                    if isinstance(tensor, torch.Tensor):
                                        batch.batch[f"token_level_scores_{name}"] = tensor
                        except Exception:
                            pass
                        ...
                        if not self.config.worker.actor.use_kl_loss:  # not grpo
                            ...
                            # GRPO: keep raw reward components as-is (no KL penalty on reward)
                            for name in [
                                "accuracy_x1",
                                "accuracy_x2",
                                "accuracy_x3",
                                ...
                                k = f"token_level_scores_{name}"
                                if k in batch.batch.keys():
                                    batch.batch[f"token_level_rewards_{name}"] = batch.batch[k]
                        ...
                        # Optional: compute GRPO advantages for reward components (x1/x2/x3) for gradient attribution
                        if self.config.algorithm.adv_estimator == "grpo":
                            try:
                                ...
                                if index is not None:
                                    ...
                                    response_mask = attention_mask[:, -response_length:]
                                    for name in ["accuracy_x1", "accuracy_x2", "accuracy_x3"]:
                                        rk = f"token_level_rewards_{name}"
                                        if rk in batch.batch.keys():
                                            adv, _ = core_algos.compute_grpo_outcome_advantage(
                                                token_level_rewards=batch.batch[rk],
                                                eos_mask=response_mask,
                                                index=index,
                                            )
                                            batch.batch[f"advantages_{name}"] = adv

                                    # Optional: raw (pre-serve) reward attributions
                                    raw_names = ["accuracy_raw_x1", "accuracy_raw_x2", "accuracy_raw_x3"]
                                    raw_present = [n for n in raw_names if f"token_level_rewards_{n}" in batch.batch.keys()]
                                    if raw_present:
                                        ...
                                        # Build a "raw total reward" tensor for GRPO (format + non_repeat + raw accuracy sum)
                                        raw_total = None
                                        try:
                                            for name in ["format", "non_repeat"]:
                                                rk = f"token_level_rewards_{name}"
                                                if rk in batch.batch.keys():
                                                    raw_total = batch.batch[rk] if raw_total is None else (raw_total + batch.batch[rk])
                                            for name in raw_present:
                                                rk = f"token_level_rewards_{name}"
                                                raw_total = batch.batch[rk] if raw_total is None else (raw_total + batch.batch[rk])
                                        except Exception:
                                        ...
                                        if isinstance(raw_total, torch.Tensor):
                                            batch.batch["token_level_rewards_raw_total"] = raw_total
                                            adv_raw_total, _ = core_algos.compute_grpo_outcome_advantage(
                                                token_level_rewards=raw_total,
                                                eos_mask=response_mask,
                                                index=index,
                                            )
                                            batch.batch["advantages_raw_total"] = adv_raw_total
                            except Exception:
                                pass
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        # === 在这里加 flush ===
                        if int(os.getenv("QUANTILE_DO_FLUSH", "1")):
                            c = _get_quant_client()
                            if c is not None:
                                try:
                                    res = c.flush()
                                    # 记录到 metrics，方便监控
                                    metrics.update({
                                        "quantile/flush_count": res.get("flush_count", 0),
                                        "quantile/main_len":    res.get("new_len", 0),
                                        "quantile/added":       res.get("added", 0),
                                        "quantile/dropped":     res.get("dropped", 0),
                                    })
                                except Exception:
                                    metrics["quantile/flush_error"] = 1
                ...
                # Optional breakpoint / stop after a "big batch" (one dataloader batch / global step).
                break_flag = str(os.getenv("VERL_BREAK_AFTER_BIG_BATCH", "0")).strip().lower() not in (
                    "",
                    "0",
                    "false",
                    "no",
                ...
                if break_flag:
                    try:
                        break_step = int(os.getenv("VERL_BREAK_AFTER_BIG_BATCH_STEP", "1"))
                    except Exception:
                        break_step = 1

                    if self.global_steps == break_step:
                        mode = os.getenv("VERL_BREAK_AFTER_BIG_BATCH_MODE", "pdb").strip().lower()
                        if mode == "raise":
                            raise RuntimeError(f"VERL_BREAK_AFTER_BIG_BATCH at step={self.global_steps}")
                        if sys.stdin is None or not sys.stdin.isatty():
                            raise RuntimeError(
                                f"VERL_BREAK_AFTER_BIG_BATCH requested pdb, but stdin is not a TTY (step={self.global_steps}); "
                                f"set VERL_BREAK_AFTER_BIG_BATCH_MODE=raise to stop cleanly."
                            )
                        import pdb
