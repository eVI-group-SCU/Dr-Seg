import re
import json
from scipy.optimize import linear_sum_assignment
import numpy as np

import os
from drr_module.serve import QuantileServiceClient
"dr_seg_version2.py"

_QUANT_CLIENT = None
def _get_quant_client():
    """每个进程懒加载一个 client；失败则返回 None。"""
    global _QUANT_CLIENT
    if QuantileServiceClient is None:
        return None
    if _QUANT_CLIENT is None:
        try:
            _QUANT_CLIENT = QuantileServiceClient(
                host=os.getenv("QUANTILE_HOST", "127.0.0.1"),
                port=int(os.getenv("QUANTILE_PORT", "50070")),
                authkey=os.getenv("QUANTILE_AUTH", "secret").encode("utf-8"),
                timeout_s=float(os.getenv("QUANTILE_TIMEOUT_S", "1.0")),
            )
            _QUANT_CLIENT.connect()
        except Exception:
            _QUANT_CLIENT = None
    return _QUANT_CLIENT

def _piecewise_point_score(d: float) -> float:
    """
    x3 的单对分数：
      d <= 30     -> 1
      30 < d < 200 -> (200 - d) / 170
      d >= 200    -> 0
    """
    try:
        d = float(d)
    except Exception:
        return 0.0
    if d <= 30.0:
        return 1.0
    if d >= 200.0:
        return 0.0
    return (200.0 - d) / 170.0


def vision_reasoner_format_reward(predict_str: str) -> float:
    pattern = r"<think>.*?</think>\s*<answer>.*?</answer>"
    match = re.fullmatch(pattern, predict_str, re.DOTALL)
    thinking_format_reward = 1.0 if match else 0.0 
    
    def segmentation_format(predict_str: str) -> float:
        segmentation_format_reward = 0.0
        try:
            json_match = re.search(r'<answer>\s*(.*?)\s*</answer>', predict_str, re.DOTALL)
            if not json_match:
                return segmentation_format_reward
            data = json.loads(json_match.group(1))
            
            data_cnt = len(data)
            
            for item in data:
                cur_reward = 0.0

                if 'bbox_2d' in item:
                    bbox_2d = item['bbox_2d']
                    if isinstance(bbox_2d, list) and len(bbox_2d) == 4:
                        cur_reward += 1.0
                    
                if 'point_2d' in item:
                    point_2d = item['point_2d']
                    if isinstance(point_2d, list) and len(point_2d) == 2:
                        cur_reward += 1.0
                
                segmentation_format_reward += cur_reward / data_cnt
        except Exception:
            pass
        return segmentation_format_reward

    #增加look reward,如果用<look></look>包裹给1分
    def look_format(predict_str: str) -> float:
        def _section(body: str, tag: str) -> str:
            m = re.search(fr'<{tag}>\s*(.*?)\s*</{tag}>', body, re.DOTALL)
            return m.group(1) if m else ""

        think_body = _section(predict_str, "think")
        answer_body = _section(predict_str, "answer")

        pat = r'<look>\s*\S[\s\S]*?</look>'  # 非空内容
        has_look = (
            re.search(pat, think_body, re.DOTALL) is not None or
            re.search(pat, answer_body, re.DOTALL) is not None
        )
        return 1.0 if has_look else 0.0

    segmentation_format_reward = segmentation_format(predict_str)
    look_format_reward = look_format(predict_str)
    #return thinking_format_reward + segmentation_format_reward 
    return thinking_format_reward + segmentation_format_reward + look_format_reward
    

# def vision_reasoner_accuracy_reward(predict_str: str, ground_truth: str) -> float:
#     max_accuracy_reward = 0.0
#     MAX_OBJECTS = 120  # 设置上限
    
#     try:
#         gt_data = json.loads(ground_truth)
#         gt_bboxes = [item['bbox_2d'] for item in gt_data]
#         gt_points = [item['point_2d'] for item in gt_data]
            
#         #json_match = re.search(r'```json\s*(.*?)\s*```', predict_str, re.DOTALL)
#         json_match = re.search(r'<answer>\s*(.*?)\s*</answer>', predict_str, re.DOTALL)
#         if json_match:
#             data = json.loads(json_match.group(1))
#             pred_bboxes = [item['bbox_2d'] for item in data]
#             pred_points = [item['point_2d'] for item in data]
            
#             # 只有当预测或真实值超过上限时才截断
#             if len(pred_bboxes) > MAX_OBJECTS:
#                 pred_bboxes = pred_bboxes[:MAX_OBJECTS]
#                 pred_points = pred_points[:MAX_OBJECTS]
            
#             if len(gt_bboxes) > MAX_OBJECTS:
#                 gt_bboxes = gt_bboxes[:MAX_OBJECTS]
#                 gt_points = gt_points[:MAX_OBJECTS]
            
#             # 预处理数据为numpy数组
#             pred_bboxes = np.array(pred_bboxes)  # (M,4)
#             pred_points = np.array(pred_points)  # (M,2)
#             gt_bboxes = np.array(gt_bboxes)    # (N,4)
#             gt_points = np.array(gt_points)     # (N,2)
            
#             # 并行计算所有指标
#             iou_matrix = batch_iou(pred_bboxes, gt_bboxes)  # (M,N)
#             l1_matrix = batch_l1_distance(pred_bboxes, gt_bboxes)  # (M,N)
#             points_dist_matrix = batch_points_distance(pred_points, gt_points)  # (M,N)
#             points_in_box = batch_points_in_box(pred_points, pred_bboxes)  # (M,)
            
#             # 计算reward矩阵
#             iou_reward = (iou_matrix > 0.5).astype(float)
#             bbox_l1_reward = (l1_matrix < 10).astype(float)
#             point_reward = ((points_dist_matrix < 30) & points_in_box[:,np.newaxis]).astype(float)
            
#             # 构建最终的cost矩阵
#             cost_matrix = 3.0 - (iou_reward + bbox_l1_reward + point_reward)
            
#             # 使用匈牙利算法找最优匹配
#             row_indices, col_indices = linear_sum_assignment(cost_matrix)
            
#             # 直接从cost_matrix计算总reward
#             total_reward = len(row_indices) * 3.0 - cost_matrix[row_indices, col_indices].sum()
            
#             # 计算平均reward
#             max_length = max(len(pred_bboxes), len(gt_bboxes))
#             max_accuracy_reward = total_reward / max_length
            
#     except Exception:
#         pass
#     return max_accuracy_reward

def vision_reasoner_accuracy_reward(predict_str: str, ground_truth: str) -> float:
    """
    先计算 (x1, x2, x3)，推入服务缓冲（enqueue），再用该点 query 获取 (r1,r2,r3)，
    返回 r1+r2+r3。服务异常时返回 0.0。
    定义：
      x1 = 现有匹配（沿用原 cost_matrix/匈牙利匹配）的平均 IoU（无匹配=0）
      x2 = min(#pred, #gt) / max(#pred, #gt)，两者都 0 时定义为 1.0
      x3 = 匹配对的点距分段分数的均值（见 _piecewise_point_score），无匹配=0
    """
    # 解析 GT
    try:
        gt_data = json.loads(ground_truth)
        gt_bboxes = [item['bbox_2d'] for item in gt_data]
        gt_points = [item['point_2d'] for item in gt_data]
    except Exception:
        gt_bboxes, gt_points = [], []

    # 解析预测
    data = None
    m = re.search(r'<answer>\s*(.*?)\s*</answer>', predict_str, re.DOTALL)
    if m:
        try:
            data = json.loads(m.group(1))
        except Exception:
            data = None

    pred_bboxes, pred_points = [], []
    if isinstance(data, list):
        try:
            pred_bboxes = [item['bbox_2d'] for item in data]
            pred_points = [item['point_2d'] for item in data]
        except Exception:
            pred_bboxes, pred_points = [], []

    # 截断（保持与你原逻辑一致）
    MAX_OBJECTS = 120
    if len(pred_bboxes) > MAX_OBJECTS:
        pred_bboxes = pred_bboxes[:MAX_OBJECTS]
        pred_points = pred_points[:MAX_OBJECTS]
    if len(gt_bboxes) > MAX_OBJECTS:
        gt_bboxes = gt_bboxes[:MAX_OBJECTS]
        gt_points = gt_points[:MAX_OBJECTS]

    M, N = len(pred_bboxes), len(gt_bboxes)

    # x2：数量贴合度
    x2 = 1.0 if (M == 0 and N == 0) else (min(M, N) / max(1, max(M, N)))

    # 默认 x1/x3
    x1, x3 = 0.0, 0.0

    # 有效才计算匹配与 x1/x3
    if M > 0 and N > 0:
        try:
            pred_bboxes_np = np.array(pred_bboxes, dtype=float)
            pred_points_np = np.array(pred_points, dtype=float)
            gt_bboxes_np   = np.array(gt_bboxes,   dtype=float)
            gt_points_np   = np.array(gt_points,   dtype=float)

            # 矩阵
            iou_matrix         = batch_iou(pred_bboxes_np, gt_bboxes_np)            # (M,N)
            l1_matrix          = batch_l1_distance(pred_bboxes_np, gt_bboxes_np)    # (M,N)
            points_dist_matrix = batch_points_distance(pred_points_np, gt_points_np) # (M,N)
            points_in_box      = batch_points_in_box(pred_points_np, pred_bboxes_np) # (M,)

            # 沿用你原来的匹配规则
            iou_reward     = (iou_matrix > 0.5).astype(float)
            bbox_l1_reward = (l1_matrix < 10).astype(float)
            point_reward   = ((points_dist_matrix < 30) & points_in_box[:, np.newaxis]).astype(float)
            cost_matrix    = 3.0 - (iou_reward + bbox_l1_reward + point_reward)

            row_idx, col_idx = linear_sum_assignment(cost_matrix)

            if len(row_idx) > 0:
                # x1：匹配对平均 IoU
                x1 = float(np.mean(iou_matrix[row_idx, col_idx]))

                # x3：匹配对点距分段分数均值（不再要求 in_box）
                matched_dists = points_dist_matrix[row_idx, col_idx]
                x3 = float(np.mean([_piecewise_point_score(float(d)) for d in matched_dists.ravel()]))

        except Exception:
            x1, x3 = 0.0, 0.0

    # 与服务交互：先 enqueue 后 query
    r1 = r2 = r3 = 0.0
    enqueue_ok = 0
    query_ok = 0
    c = _get_quant_client()
    if c is None:
        try:
            vision_reasoner_accuracy_reward.latest_breakdown = {
                "accuracy/x1": float(x1),
                "accuracy/x2": float(x2),
                "accuracy/x3": float(x3),
                "accuracy/r1": float(r1),
                "accuracy/r2": float(r2),
                "accuracy/r3": float(r3),
                "accuracy/enqueue_ok": int(enqueue_ok),
                "accuracy/query_ok": int(query_ok),
                "accuracy/pred_count": int(M),
                "accuracy/gt_count": int(N),
            }
        except Exception:
            pass
        return 0.0

    try:
        c.enqueue_batch([(x1, x2, x3)])  # push 到辅助缓冲
        enqueue_ok = 1
    except Exception:
        # 忽略入队错误，继续尝试查询
        pass

    out = 0.0
    try:
        r1, r2, r3 = c.query((x1, x2, x3))  # 取回 reward 三元组
        r1, r2, r3 = float(r1), float(r2), float(r3)
        out = float(r1 + r2 + r3)
        query_ok = 1
    except Exception:
        r1 = r2 = r3 = 0.0
        out = 0.0

    try:
        vision_reasoner_accuracy_reward.latest_breakdown = {
            "accuracy/x1": float(x1),
            "accuracy/x2": float(x2),
            "accuracy/x3": float(x3),
            "accuracy/r1": float(r1),
            "accuracy/r2": float(r2),
            "accuracy/r3": float(r3),
            "accuracy/enqueue_ok": int(enqueue_ok),
            "accuracy/query_ok": int(query_ok),
            "accuracy/pred_count": int(M),
            "accuracy/gt_count": int(N),
        }
    except Exception:
        pass

    return out



# def vision_reasoner_non_repeat_reward(predict_str: str) -> float:
#     non_repeat_reward = 1.0  # 初始满分
#     try:
#         sentences = predict_str.split('.')
        
#         # 移除空句子
#         sentences = [s.strip() for s in sentences if s.strip()]
        
#         # 检查重复
#         seen = set()
#         repeats = 0
        
#         for sentence in sentences:
#             if sentence in seen:
#                 repeats += 1
#             if repeats >=2:
#                 non_repeat_reward = 0
#                 break
#             seen.add(sentence)
            
#     except Exception:
#         pass
    
#     return non_repeat_reward

import re

# 允许前后有符号，也允许词与词之间有空白/标点
_BLOCK_PATTERNS = [
    re.compile(r"\byour[\s\W]*reasoning[\s\W]*text\b", re.IGNORECASE),
    re.compile(r"\byour[\s\W]*visual[\s\W]*focus\b", re.IGNORECASE),
    re.compile(r"\bmore[\s\W]*reasoning[\s\W]*text\b", re.IGNORECASE),
]

def vision_reasoner_non_repeat_reward(predict_str: str) -> float:
    """
    满分 1.0；若命中占位短语（大小写不敏感，前后可带符号）或出现 >=2 个重复句子，则记 0。
    """
    try:
        # 1) 占位短语拦截
        if any(p.search(predict_str) for p in _BLOCK_PATTERNS):
            return 0.0

        # 2) 分句：支持 . ! ? 以及中文句号/感叹/问号和省略号
        parts = re.split(r"[\.。!?？！…]+", predict_str)

        # 3) 规范化句子：去首尾空白与常见括号标点，合并多空格，统一小写
        def _norm(s: str) -> str:
            s = s.strip()
            s = s.strip(" \"'“”‘’[](){}，,;:、")
            s = re.sub(r"\s+", " ", s)
            return s.lower()

        sentences = []
        for seg in parts:
            n = _norm(seg)
            if n:
                sentences.append(n)

        # 4) 检查重复（忽略大小写与轻微标点差异）；出现两个重复即0分
        seen = {}
        repeats = 0
        for s in sentences:
            if s in seen:
                seen[s] += 1
                repeats += 1
                if repeats >= 2:
                    return 0.0
            else:
                seen[s] = 1

    except Exception:
        pass

    return 1.0



    
def dr_seg_compute_score(predict_str: str, ground_truth: str) -> float:
    # print(predict_str, ground_truth)
    format_reward = vision_reasoner_format_reward(predict_str)
    accuracy_reward = vision_reasoner_accuracy_reward(predict_str, ground_truth)
    non_repeat_reward = vision_reasoner_non_repeat_reward(predict_str)

    reward = format_reward + accuracy_reward + non_repeat_reward

    try:
        breakdown = {
            "format_reward": float(format_reward),
            "accuracy_reward": float(accuracy_reward),
            "non_repeat_reward": float(non_repeat_reward),
            "total_reward": float(reward),
        }
        acc_bd = getattr(vision_reasoner_accuracy_reward, "latest_breakdown", None)
        if isinstance(acc_bd, dict):
            breakdown.update(acc_bd)
        dr_seg_compute_score.latest_breakdown = breakdown
    except Exception:
        pass

    return reward

def batch_iou(boxes1, boxes2):
    # boxes1: (M,4), boxes2: (N,4)
    # 广播机制自动扩展维度
    x11, y11, x12, y12 = np.split(boxes1, 4, axis=1)  # (M,1)
    x21, y21, x22, y22 = np.split(boxes2, 4, axis=1)  # (N,1)
    
    xA = np.maximum(x11, np.transpose(x21))  # (M,N)
    yA = np.maximum(y11, np.transpose(y21))
    xB = np.minimum(x12, np.transpose(x22))
    yB = np.minimum(y12, np.transpose(y22))
    
    interArea = np.maximum(0, xB - xA + 1) * np.maximum(0, yB - yA + 1)
    box1Area = (x12 - x11 + 1) * (y12 - y11 + 1)  # (M,1)
    box2Area = (x22 - x21 + 1) * (y22 - y21 + 1)  # (N,1)
    
    unionArea = box1Area + np.transpose(box2Area) - interArea
    iou = interArea / unionArea  # (M,N)
    return iou

def batch_l1_distance(boxes1, boxes2):
    # boxes1: (M,4), boxes2: (N,4)
    boxes1 = boxes1[:, np.newaxis, :]  # (M,1,4)
    boxes2 = boxes2[np.newaxis, :, :]  # (1,N,4)
    return np.mean(np.abs(boxes1 - boxes2), axis=2)  # (M,N)

def batch_points_distance(points1, points2):
    # points1: (M,2), points2: (N,2)
    points1 = points1[:, np.newaxis, :]  # (M,1,2)
    points2 = points2[np.newaxis, :, :]  # (1,N,2)
    
    # 计算欧氏距离
    dist = np.sqrt(np.sum((points1 - points2)**2, axis=2))  # (M,N)
    return dist

def batch_points_in_box(points, boxes):
    """
    检查每个点是否在对应的框内
    points: (M,2) - M个点的坐标
    boxes: (M,4) - M个框的坐标 [x1,y1,x2,y2]
    返回: (M,) 布尔数组
    """
    x_check = (points[:,0] >= boxes[:,0]) & (points[:,0] <= boxes[:,2])
    y_check = (points[:,1] >= boxes[:,1]) & (points[:,1] <= boxes[:,3])    
    return x_check & y_check



if __name__ == "__main__":
    predict_str = """
<answer>
[{"bbox_2d": [10, 100, 398, 423], "point_2d": [283, 169]}]
</answer>
"""
    ground_truth = """
[{"bbox_2d": [416, 7, 833, 553], "point_2d": [648, 249]}]"""
    print(predict_str)
    print(ground_truth)
    print(dr_seg_compute_score(predict_str, ground_truth))
    
